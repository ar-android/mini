...
void enabledRawMode(){
    if(tcgetattr(STDIN_FILENO, &E.orig_termios) ==  -1) die("tgetattr");
...
int getWindowSize(int *rows, int *cols){
    ...
    if(ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0){
        if (write(STDOUT_FILENO, "\x1b[999C\x1b[99B", 12) != 12) return -1;
...
void editorOpen(char *filename) {
    free(E.filename);
    E.filename = (char*)strdup(filename);
    ...
    for ( y = 0; y < E.screenrows; y++){
        ...
        if (filerow >= E.numrows) {
            if (E.numrows == 0 && y == (E.screenrows / 2)){
                ...
                "Mini editor -- version %s", APP_VERSION);
